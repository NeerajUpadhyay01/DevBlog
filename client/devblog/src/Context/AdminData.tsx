...
export const AdminContextProvider = (props: any) => {
  const [status, setStatus] = useState<credentialinfo>({
    success: false,
    message: "",
  });
  const storedAdmin = localStorage.getItem("admin");
  const initialAdmin = storedAdmin ? JSON.parse(storedAdmin) : null;
  const [storeAdmin, setStoreAdmin] = useState<adminInfo | null>(
    initialAdmin || null
  );
  ...
  const AdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/admin/login",
        {
          method: "POST",
  ...
  useEffect(() => {
    ...
    document.cookie != "" && storeAdmin != null
      ? setStatus({ success: true, message: "Login successfully" })
