...
export const UserContextProvider = (props: any) => {
  ...
  const reducer = async (
    state: String,
    action: { type: string; data?: any; id?: String }
  ) => {
    switch (action.type) {
      case "LOGIN": {
        try {
          ...
          const response = await fetch(
            "https://devblog-kr5o.onrender.com" + "/user/login",
            {
              method: "POST",
      ...
      case "UNSUBSCRIBE": {
        try {
          const response = await fetch(
            "https://devblog-kr5o.onrender.com" + "/unsubscribe/" + action.id,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
            }
  ...
  useEffect(() => {
    document.cookie
      ? setLoginStatus({ success: true, message: "User login" })
      : setLoginStatus({ success: false, message: "Please Login" });
  }, []);
