...
export const PostContextProvider = (props: any) => {
  const [postStatus, setPostStatus] = useState<datastatus>({
    success: false,
    message: "",
  });
  const [allposts, SetAllPosts] = useState<allblogs[]>();
  const [deletePost, setDeletePost] = useState<datastatus>({
    success: false,
    message: "",
  });
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/api/posts",
        {
          method: "GET",
          headers: {
  ...
  const DeletePost = async (PostId: String) => {
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/delete/post/" + PostId,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const FlagPost = async (PostId: String) => {
    ...
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/flag/post/" + PostId,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <postContext.Provider value={info}>{props.children}</postContext.Provider>
  );
};
