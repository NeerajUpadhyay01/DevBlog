  ...
  const hanldeSubmit = async (key: boolean) => {
    ...
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/publish/blogpost",
        {
          method: "POST",
      ...
      <div className="h-max w-[100vw] flex flex-col items-center gap-5 p-3">
        {status.success === false && document.cookie ? (
          <>
            ...
            <form
              method="POST"
              className="h-max w-max flex flex-col items-center gap-5"
            >
              <span className="text-2xl text-indigo-600 font-medium">
                Title
              </span>
              <input
                className="border-indigo-600 border rounded shadow w-[75vw] h-[7vh] px-2"
                type="text"
                value={blog.title.toString()}
              ...
              <div className="bg-indigo-600 rounded w-[65vw] h-max p-3 text-white items-center justify-evenly flex flex-col">
                ...
                {blog.image === "" || blog.image === null ? (
                  ...
                  <img height={500} width={700} src={blog.image} />
                )}
              </div>
              <span className="text-2xl text-indigo-600 font-medium">
                Description
              ...
              <textarea
                className="border-indigo-600 border rounded shadow w-[75vw] h-max p-1"
                value={blog.meta.toString()}
                onChange={handlechange}
                name="meta"
              />
              <span className="text-2xl text-indigo-600 font-medium">
          ...
          <div className="h-[100vh] w-[100vw] flex items-center justify-center">
            {document.cookie ? (
              <p className="text-indigo-600 title md:text-2xl text-xl font-bold text-center">
                {status.message}
              </p>
            ) : (
              <p className="text-indigo-600 title md:text-2xl text-xl font-bold text-center">
