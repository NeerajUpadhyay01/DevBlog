...
const OTPverification: React.FC = () => {
  ...
  const [status, setStatus] = useState<statusType>({
    success: false,
    message: "OTP sent, Please Check your inbox/spam folder",
  });
  ...
  const ResendOtp = async (UserId: String | undefined) => {
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/verify/account/re/" + UserId,
        {
          method: "GET",
          headers: {
  ...
  const OtpSubmit = async (UserId: String | undefined) => {
    ...
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/verify/account/" + UserId,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className="h-[100vh] w-[100vw] flex flex-col justify-center items-center gap-5 p-3">
      ...
      <div className="flex flex-col justify-around gap-5 border shadow-md w-max h-max p-4">
        <div className="h-max w-max p-3 flex flex-col gap-5">
          <div className="text-2xl text-indigo-600 font-semibold">OTP</div>
          <div>
            <input
              className="px-2 h-[2.25rem] w-[65vw] md:w-[25vw] border rounded"
              type="number"
          ...
          <div className="flex flex-row flex-wrap justify-around text-base">
            <p>Resend the OTP?</p>
            <button
              className="bg-indigo-600 shadow-md rounded-sm text-md px-1 text-white"
              onClick={() => ResendOtp(regitserInfo?.registeredUserId)}
            >
              Resend
            </button>
          </div>
          <div className="text-center">
            <button
              className="bg-indigo-600 w-[20vw] shadow-md rounded-sm p-2 text-md font-semibold text-white"
              onClick={() => OtpSubmit(regitserInfo?.registeredUserId)}
            >
              Submit
            </button>
          </div>
