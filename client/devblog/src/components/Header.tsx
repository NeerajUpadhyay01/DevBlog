...
const Header: React.FC = () => {
  ...
  return (
    <>
          ...
          <Link to="/">
            <span className="h-max flex items-center gap-2">
              <Icon
                ...
                height="4vh"
              />
              <h1 className="logo md:text-2xl text-xl text-black font-bold">
          ...
          <ul className="hidden md:flex w-[100%] flex-row justify-evenly items-center font-semibold">
            ...
            {admincontext?.status.success === false ? (
              usercontext?.loginstatus.success == false ? (
                <li className="hover:text-indigo-600">
                  <Link to="/register">
                    <button className="bg-indigo-600 rounded-sm p-2 text-sm text-white hover:shadow-md transition duration-300 ease-in-out transform hover:-translate-y+1 hover:scale-105">
                      Login/SignUp
                    </button>
                  </Link>
                </li>
                ...
                <li className="hover:text-indigo-600">
                  <button
                    ...
                    className="bg-indigo-600 rounded-sm p-1 text-sm text-white hover:shadow-md"
                  >
                    Logout
                  </button>
                </li>
              ...
              <li className="hover:text-indigo-600">
                <button
                  onClick={() => {
                    admincontext?.dispatch({ type: "ADMINLOGOUT" });
                  }}
                  className="bg-indigo-600 rounded-sm p-1 text-sm text-white hover:shadow-md"
                >
                  Logout
      ...
      {mobileDevice == true ? (
        <div className="absolute md:hidden block h-max w-screen bg-indigo-600 items-center p-3">
