...
const ProfilePost: React.FC<PropsType> = ({
  id,
  image,
  title,
  meta,
  ...
  const LoadPost = async (Postid: String) => {
    try {
      const response = await fetch(
        "https://devblog-kr5o.onrender.com" + "/view/post/" + Postid,
        {
          method: "GET",
          headers: {
      ...
      <div className="border-2 p-3 rounded border-indigo-600 h-max w-[80vw] flex md:flex-row flex-col gap-5 items-center justify-center">
        <div className="relative md:w-[20vw] w-[65vw] h-max">
          <span className="absolute">
            {publicView === true ? (
              <p className="bg-green-500 p-0.5 text-white w-max text-sm font-light">
                Public
              </p>
            ) : (
              <p className="bg-red-600 p-0.5 text-white w-max text-sm font-light">
                Private
              </p>
            )}
          </span>
        ...
        <div className="flex md:flex-col flex-row flex-wrap justify-center items-center gap-5">
          <button
            onClick={() => LoadPost(id)}
            className="bg-indigo-600 md:w-[8vw] w-[25vw] flex items-center justify-center gap-1 font-medium shadow-md rounded-sm px-1 text-base text-white"
          >
            <Icon icon="iconamoon:edit-light" color="white" /> Read
          </button>
          {admin === true ? (
            <Link to={`/flag/${id}`}>
              <button className="bg-orange-600 md:w-[8vw] w-[25vw] flex items-center justify-center gap-1 font-medium shadow-md rounded-sm px-1 text-base text-white">
            ...
            <button
              onClick={() => EditPost(id)}
              className="bg-green-500 md:w-[8vw] w-[25vw] flex items-center justify-center gap-1 font-medium shadow-md rounded-sm px-1 text-base text-white"
            >
              <Icon icon="iconamoon:edit-light" color="white" /> Edit
            </button>
          )}
          <button
            onClick={() => post?.DeletePost(id)}
            className="bg-red-600 md:w-[8vw] w-[25vw] flex items-center justify-center gap-1 font-medium shadow-md rounded-sm px-1 text-base text-white"
          >
